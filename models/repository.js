...
class Repository {
    ...
    getAll(params = null) {
        ...
        if (params) {
            ...
            Object.keys(params).forEach(function (paramName) {
                if (paramName == "sort") {
                    let keyValues = params[paramName];//key value devient name
            ...
            // todo filter
            for (var object of objectsList) {//of au lieu de in
            ...
            if (sortKeys.length > 0)
                if(searchKeys.length > 0)
                    filteredAndSortedObjects = this.orderObjectList(filteredAndSortedObjects, sortKeys);
                else
                    filteredAndSortedObjects = this.orderObjectList(objectsList, sortKeys);
    ...
    compare(itemX, itemY) {
        ...
        do {
            let result = 0;
            if (this.sortFields[fieldIndex].asc)//erreur si pas asc
    ...
    orderObjectList(objectsToFilter, sortKeys) {
        this.sortFields = sortKeys;
        let SortedObjects = [...objectsToFilter].sort((x, y) => {//créer une nouvelle référence
